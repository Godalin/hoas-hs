...
data Term
  = App !Term !Term
  | Lam (Term -> Term)
  | Let !Term (Term -> Term) -- for let bindings
  | Fix (Term -> Term) -- fixed-point combinator
...
reduceCBN (App (Lam f) arg) = Step (f arg) -- function arguments are not early reduced
...
reduceCBN (App t1 t2) =
  case reduceCBN t1 of
    ...
    Error err -> Error err
reduceCBN (Let t f) =
  case reduceCBN t of -- let bindings are early reduced
    Stop t' -> Step (f t')
    Step t' -> Step (Let t' f)
    Error err -> Error err
